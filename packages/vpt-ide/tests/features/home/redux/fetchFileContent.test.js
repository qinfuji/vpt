...
describe('home/redux/fetchFileContent', () => {
  ...
  it('dispatches success action when fetchFileContent succeeds', () => {
    ...
    return store.dispatch(fetchFileContent('file')).then(() => {
      ...
      expect(actions[1]).to.have.nested.property(
        'data.content',
        'file content'
      );
    });
  ...
  it('dispatches failure action when fetchFileContent fails', () => {
    ...
    return store.dispatch(fetchFileContent('file')).catch(() => {
      const actions = store.getActions();
      expect(actions[0]).to.have.property(
        'type',
        HOME_FETCH_FILE_CONTENT_BEGIN
  ...
  it('handles action type HOME_FETCH_FILE_CONTENT_BEGIN correctly', () => {
    ...
    const state = reducer(prevState, { type: HOME_FETCH_FILE_CONTENT_BEGIN });
