...
import { REKIT_TOOLS_CLEAR_TEST_OUTPUT } from 'src/features/rekit-tools/redux/constants';
...
describe('rekit-tools/redux/clearTestOutput', () => {
  it('returns correct action by clearTestOutput', () => {
    expect(clearTestOutput()).to.have.property(
      'type',
  ...
  it('handles action type REKIT_TOOLS_CLEAR_TEST_OUTPUT correctly', () => {
    ...
    const state = reducer(prevState, { type: REKIT_TOOLS_CLEAR_TEST_OUTPUT });
