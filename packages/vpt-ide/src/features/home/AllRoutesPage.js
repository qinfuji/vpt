...
export class AllRoutesPage extends Component {
  ...
  render() {
    ...
    const routes = _.flow(_.values, pr(_.map, 'routes'), _.flatten)(
      featureById
    );
    ...
    return (
      <div className="home-all-routes-page">
        <dl>
          {features
            .filter(fid => featureById[fid].routes.length > 0)
            .map(fid => [
...
export default connect(mapStateToProps, mapDispatchToProps)(AllRoutesPage);
