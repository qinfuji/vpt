...
} from './selectors/explorerTreeData';
...
export class ProjectExplorer extends Component {
  ...
  handleContextMenu = evt => {
    ...
    const x =
      evt.event.clientX - this.rootNode.offsetLeft + this.rootNode.scrollLeft; // eslint-disable-line
    const y =
      evt.event.clientY - this.rootNode.offsetTop + this.rootNode.scrollTop; // eslint-disable-line
    ...
    switch (evt.key) {
      ...
      case 'run-tests':
        if (cmdContext.elementType === 'features') {
          ...
          // components, actions
          history.push(
            `/tools/tests/${cmdContext.feature}%2F${cmdContext.elementType}`
          );
        ...
        Modal.confirm({
          ...
          content:
            cmdContext.elementType === 'feature'
              ...
              : `Delete ${cmdContext.elementType}: ${cmdContext.feature}/${
                  cmdContext.elementName
                } ? `,
          onOk: () => {
            const hide = message.loading(
              `Deleting ${cmdContext.elementName}`,
              0
            );
            this.props.actions
              .execCmd({
  ...
  renderContextMenu() {
    return (
      <Menu
          ...
          style={{ minWidth: 150 }}
      >
        {this.state.contextMenu.map(menuItem => (
          <Menu.Item key={menuItem.key}>{menuItem.name}</Menu.Item>
        ))}
      </Menu>
    );
  }
    ...
    return (
      <span>
        ...
        <span className="search-highlight">
          {label.substring(i, i + searchKey.length)}
        </span>
  ...
  renderTreeNodeTitle(nodeData) {
    ...
    return (
      <span>
        ...
        <label>
          {nodeData.searchable
            ? this.renderHighlightedTreeNodeLabel(nodeData.label)
            : nodeData.label}
